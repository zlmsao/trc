...
NioApp = function (NioApp, $, window, document) {
	...
	if (window.location.hash === '#subscribe') {
		...
		toastr.info('Subscription Confirmed!');
	}
...
You'll need a VPN with the ability to change countries to accurately use this. You will not see an alert unless your IP appears to be from somewhere in the EU.

If you don't have a VPN, you can plug this url (that includes an IP from Paris, France) into the GET request below: https://api.ipdata.co/194.187.249.49?api-key=test&format=jsonp

Uses Test API Key from ipdata.co, you'll need your own key to use in production. It's free with an account for up to 1000 requests per day. Read the API docs here: https://docs.ipdata.co/

Uses jQuery because I'm feeling lazy and the project I'm testing for already uses it. Can easily be converted to vanilla js.
*/


// Function to close alert, uses .del as CSS class
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Gets IP Geolocation Data from ipdata.co API
  // If you don't have a VPN, you can plug this url into the GET request: https://api.ipdata.co/194.187.249.49?api-key=test&format=jsonp
  $.get(
      "https://api.ipdata.co/194.187.249.49?api-key=test&format=jsonp",
      function(response) {
          
          // if IP is located in European Union
          if (response.is_eu == true) {
              
              // Get Cookie
              var euCookie = getEUCookie("euip");			  
  ...
  // Set Cookie Function
  function setEUCookie(cname, cvalue, exdays) {
      var d = new Date();
